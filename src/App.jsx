...
async function parseExcelFile(file) {
  ...
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    // Riga con data valida (oggetto Date o stringa data)
    const rawDate = row[0];
    if (!rawDate || typeof rawDate !== "object" || !(rawDate instanceof Date)) continue;
    const dateStr = rawDate.toISOString().split("T")[0];

    // Colonne: 0=data, 1=tipo, 2=categoria/note, 3=nome, 4=quota affitto, 5=quota catering, 6=incassato
    const tipo = row[1] ? String(row[1]).trim() : "";
    const categoria = row[2] ? String(row[2]).trim() : "";
    const nome = row[3] ? String(row[3]).trim() : "";
    const notes = [categoria, nome].filter(Boolean).join(" - ") || tipo;
    const priceLocation = row[4] ? String(Math.round(Number(row[4]))) : "";
    const priceCatering = row[5] ? String(Math.round(Number(row[5]))) : "";
    const deposit = row[6] ? String(Math.round(Number(row[6]))) : "";

    // Mappa tipo a quelli dell'app
    const tipoNorm = tipo.toLowerCase();
    let type = "Pranzo";
    if (tipoNorm.includes("cena")) type = "Cena";
    else if (tipoNorm.includes("location") || (!row[5] && !tipoNorm.includes("catering"))) type = "Pranzo";
...
export default function App() {
  ...
  const handleImportExcel = async () => {
    ...
    for (const b of importExcelPreview) await fbSet(b);
    const data = await fbGet();
    setBookings(data);
    setSyncing(false);
  ...
  const totalDeposit = filtered.reduce((s,b)=>s+Number(b.deposit||0),0);
      ...
      <div style={{background:"#3A2A1A",padding:"24px 28px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:14}}>
        ...
        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
          ...
          <button onClick={()=>setShowImport(true)}
            style={{...btnStyle,background:"#5A7A40",color:"#fff",padding:"12px 18px",fontSize:13}}>
            📥 Importa eventi 2026
          </button>
