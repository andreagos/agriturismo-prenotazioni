...
async function parseExcelFile(file) {
  ...
  // ID stabile: stessa data + stesso nome = stesso evento (niente doppioni)
  const slug = (s) => s.toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .slice(0, 40) || "evento";

  // Legge tutti i fogli tranne quelli storici (es. "2025")
  const sheetsToRead = wb.SheetNames.filter(name => !/^20\d{2}$/.test(name.trim()));
  ...
  for (const sheetName of sheetsToRead) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // colonna 4 = quota affitto, colonna 6 = quota solo affitto
      const locVal = row[4] || row[6];
      const priceLocation = locVal ? String(Math.round(Number(locVal))) : "";
      ...
      let id = `xl_${dateStr}_${slug(notes)}`;
      let n = 2;
      while (usedIds.has(id)) { id = `xl_${dateStr}_${slug(notes)}_${n}`; n++; }
      usedIds.add(id);
...
const EVENT_TYPES = ["Location", "Location+Catering"];
const COLORS = {
  "Location":           { accent: "#5A8A3A", light: "#EEF8E0", badge: "#2A5A10" },
  "Catering":           { accent: "#3A6A9A", light: "#E0F0FF", badge: "#1A3A6A" },
  "Location+Catering":  { accent: "#C2732A", light: "#FFF0E0", badge: "#7A3A00" },
};
const ICONS = { "Location": "🏡", "Catering": "🍽", "Location+Catering": "🌿" };
...
function Modal({ booking, onClose, onSave }) {
  const [form, setForm] = useState(
    booking ? { ...booking } : { date:"", type:"Location+Catering", status:"confermato", people:"", priceLocation:"", priceCatering:"", deposit:"", notes:"" }
  );
...
export default function App() {
  ...
  const handleImportExcel = async () => {
    ...
    try {
      const payload = {};
      // Rimuove tutti gli eventi importati in precedenza da Excel (id "xl_...")
      // così l'Excel resta l'unica fonte: niente doppioni.
      // Gli eventi creati a mano nell'app non vengono toccati.
      for (const b of bookings) {
        if (String(b.id).startsWith("xl_")) payload[b.id] = null;
      }
      // Poi carica quelli nuovi
  ...
  // Applica filtro attivi/passati/tutti
  let displayList = [];
  if (filterStatus === "attivi") displayList = filterByPeriod(upcoming);
  else if (filterStatus === "passati") displayList = [...filterByPeriod(past)].reverse();
  else displayList = filterByPeriod(allFiltered);

  const displayUpcoming = displayList.filter(b=>new Date(b.date)>=today);
  const displayPast = [...displayList.filter(b=>new Date(b.date)<today)].reverse();

  const statsBase = displayList;
  const filtered = statsBase;

  const totalLocation = statsBase.reduce((s,b)=>s+Number(b.priceLocation||0),0);
  const totalCatering = statsBase.reduce((s,b)=>s+Number(b.priceCatering||0),0);
          ...
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16,flexWrap:"wrap"}}>
              ...
              <div style={{display:"flex",gap:6,marginLeft:"auto"}}>
                {[{key:"attivi",label:"📅 Attivi"},{key:"passati",label:"🕰 Passati"},{key:"tutti",label:"Tutti"}].map(s=>(
                  <button key={s.key} onClick={()=>setFilterStatus(s.key)}
                    style={{...btnStyle,padding:"7px 13px",fontSize:12,
                      background:filterStatus===s.key?"#C2732A":"#fff",
                      color:filterStatus===s.key?"#fff":"#7A6A50",
                      border:"1.5px solid",borderColor:filterStatus===s.key?"#C2732A":"#DDD0BC"}}>
                    {s.label}
                  </button>
                ))}
              </div>
            ...
            {displayUpcoming.length>0 && (
              <div style={{marginBottom:22}}>
                <div style={{fontFamily:"'Playfair Display',serif",fontSize:17,color:"#3A2A1A",marginBottom:10}}>📅 Prossimi eventi ({displayUpcoming.length})</div>
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  {displayUpcoming.map(b=><BookingCard key={b.id} b={b} onEdit={openEdit} onDelete={id=>setConfirmDelete(id)} formatDate={formatDate}/>)}
                </div>
              </div>
            )}
            {displayPast.length>0 && (
              <div style={{marginBottom:22}}>
                <div style={{fontFamily:"'Playfair Display',serif",fontSize:17,color:"#9A8A70",marginBottom:10}}>🕰 Passati ({displayPast.length})</div>
                <div style={{display:"flex",flexDirection:"column",gap:10,opacity:0.7}}>
                  {displayPast.map(b=><BookingCard key={b.id} b={b} onEdit={openEdit} onDelete={id=>setConfirmDelete(id)} formatDate={formatDate}/>)}
                </div>
              </div>
            )}
